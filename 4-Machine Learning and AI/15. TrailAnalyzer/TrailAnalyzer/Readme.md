...
### TrailField 제네릭 타입
- 제네릭 타입은 특정 하나의 타입에 고정되지 않고 여러 타입을 유연하게 처리할 수 있는 타입
- TrailField는 타입 이름 뒤에 `< >`를 사용하여 제네릭 타입 매개변수를 받는 구조
- 제네릭 타입 매개변수를 통해 TrailField가 어떤 데이터 타입을 다룰지 나중에 정할 수 있음
- 같은 구조를 유지하면서도 다양한 타입에 재사용 가능

-> TrailField는 제네릭 타입 매개변수를 받기 때문에 제네릭 타입

### 모델의 출력 결과 표시
- 모델은 입력 데이터를 바탕으로 예측 결과를 생성
